...
import { IoChevronBackOutline, IoCloseOutline, IoInformationCircle } from "react-icons/io5";
import { RiDeleteBin6Line } from "react-icons/ri";
import { Loader } from "../Loader";
import { toast } from "sonner";
import { useRecrawlWebsiteData, useUpdateTrainData } from "@/utils/botCreation-api";
...
const AIKnowledge = ({
  ...
  const fetchTrainData = async () => {
    const response: TrainDataType = await axiosInstance.get(`/chatbot/${chatbotId}/chatbotDoc`);
    if (response.success) {
  ...
  const { mutate: onUpdate, isPending: isPendingToUpdate } = useUpdateTrainData({
    ...
    onError(error: axiosError) {
      const errorMessage =
        error?.response?.data?.errors?.message || error?.response?.data?.message || "Failed to update";
      toast.error(errorMessage);
    },
  });

  const { mutate: onRecrawl, isPending: isPendingToRecrawl } = useRecrawlWebsiteData({
    onSuccess(data) {
      refetchTrainData();
      setInitialIndex(0);
      toast.success(data?.message);
  ...
  const updateHandler = async () => {
    if (tab === "documents" && listOfDocument.length === 0 && listOfWebsites.length === 0) {
      ...
      const data = {
        ...
        details:
          ...
          //   :
          { websiteContent: listOfWebsites, documentContent: listOfDocument },
      };
      onUpdate(data);
  ...
  const handleDelete = (index: number) => {
    if (tab === "documents" && listOfDocument.length <= 1 && listOfWebsites.length === 0) {
  ...
  return (
    <div
      ...
      style={{
        height: screenWidth > 500 ? "calc(100dvh - 72px)" : "calc(100dvh - 96px)",
      }}
    >
      ...
      >
        ...
        >
          <div className="text-[#1E255E] text-xl font-medium text-center mb-2 md:mb-4">AI Knowledge</div>
          <div className="flex justify-center md:justify-normal gap-5 md:gap-0 md:flex-col ">
            {["websites", "documents"]
              .filter((item) => item !== "websites" || listOfWebsites.length > 0)
              .map((item) => (
                <div className="flex justify-between items-center" key={item}>
                  ...
                  <div className="text-[#7A7A7A] text-base font-medium hidden md:block">
                    {item === "websites" ? listOfWebsites.length : listOfDocument.length}
                  </div>
                </div>
              ))}
          </div>
        ...
        <div className="w-full  md:w-[70%] lg:w-[75%] xl:w-[82%] overflow-hidden px-4 sm:px-6 pt-4 sm:pt-6 mb-4 sm:mb-6 flex flex-1 gap-4 flex-col">
          {initialIndex === 0 ? (
                    ...
                    <TableRow className="w-full">
                      <TableHead className="py-2 text-start w-[40%] md:w-[50%]">
                        <div className="flex items-center flex-wrap justify-start gap-1">
                          <span className="text-[#1E255E] font-medium">
                            {tab === "websites" ? "Website  Name" : "Document Name"}
                          </span>
                        </div>
                      </TableHead>
                      ...
                      <TableHead className="py-2 text-center w-[20%] md:w-[25%]">
                        <div className="flex items-center flex-wrap justify-center">
                          <span className="text-[#1E255E] font-medium">State</span>
                        </div>
                      </TableHead>
                      <TableHead className="py-2 text-center w-[20%] md:w-[25%]">
                        <div className="flex items-center flex-wrap justify-center">
                          <span className="text-[#1E255E] font-medium">Action</span>
                        </div>
                      </TableHead>
                    </TableRow>
                  </TableHeader>
                    ...
                    {Array.isArray(getCurrentList()) && getCurrentList().length > 0 ? (
                          ...
                          <TableCell className="text-center">
                            <div
                              className={`flex font-normal  ${
                                detail.active ? "text-[#008000]" : "text-[#FF0000]"
                            ...
                            <div className="flex  justify-center gap-2 align-baseline">
                              <TooltipProvider>
                                <Tooltip>
                                  <TooltipTrigger asChild>
                                    <div>
                                      <Switch checked={detail.active} onCheckedChange={() => handleToggle(index)} />
                                    </div>
                                  </TooltipTrigger>
                                  <TooltipContent
                                    side="bottom"
                      ...
                      <TableRow>
                        <TableCell className="text-center py-4" colSpan={4}>
                          <div className="text-[#1E255E] font-normal">No data</div>
                        </TableCell>
                      </TableRow>
                    )}
                  </TableBody>
            ...
            <div className="flex-1 flex flex-col overflow-hidden">
              <div className="flex items-center gap-3">
                <IoChevronBackOutline className="text-[#1E255E] cursor-pointer" onClick={() => setInitialIndex(0)} />
                <span className="text-[#1E255E] capitalize text-lg sm:text-xl font-medium text-center ">Documents</span>
              </div>
              ...
              <div className="flex-1 flex flex-col mx-2 relative overflow-hidden">
                <div className="flex-1 overflow-y-auto ">
                  <div className="grid overflow-y-auto gap-4 sm:gap-6 grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 ">
                    {files.map((file, index) => (
                      <div key={index}>
                        <div className="border-[#CCCCCC] border border-dashed  flex flex-col items-center justify-center gap-2 w-full h-36">
                          <Image src="/images/file_pic.svg" alt="upload" width={84} height={84} quality={100} />
                        </div>
                    ...
                    <div>
                      <div className="border-[#CCCCCC] border border-dashed flex flex-col items-center justify-center gap-2 w-full h-36">
                        <Image src="/images/arrow_upload.svg" alt="upload" width={43} height={43} quality={100} />
                        <div className="text-[#7E7E7E] font-normal text-sm">PDF only, max 10MB.</div>
                      </div>
                      <label className="flex items-center justify-between border border-[#57C0DD]  w-full  p-2 cursor-pointer">
                        <input type="file" className="hidden" onChange={handleFileChange} />
                        <span className="text-sm sm:text-base w-full text-center text-[#57C0DD]">Choose file</span>
                      </label>
                    </div>
                  </div>
                </div>
        ...
        <DialogContent className="max-w-[93dvw] gap-0 sm:max-w-[425px] rounded-lg p-4 sm:p-6">
          ...
          <div className="gap-6 flex flex-col">
            <div className="flex items-center justify-between">
              <div></div>
              <div className="text-primary text-lg sm:text-xl font-medium">Detected Unsaved Changes</div>

              <IoCloseOutline className="text-lg cursor-pointer" onClick={() => setShowConfirmDialog(false)} />
            </div>

            <div className="space-y-2">
              <p className="text-center">You have unsaved changes. What would you like to do?</p>
            </div>
