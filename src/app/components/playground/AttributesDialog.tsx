...
import { useAddAttributes, useDeleteAttribute, useUpdateAttributes } from "@/utils/attributes-api";
...
const AttributesDialog = ({
  ...
  const { mutate: onAddAttributes, isPending: isAddPending } = useAddAttributes({
    ...
    onError(error: axiosError) {
      const errorMessage =
        error?.response?.data?.errors?.message || error?.response?.data?.message || "add attributes failed";
      toast.error(errorMessage);
    },
  });
  const { mutate: onUpdateAttributes, isPending: isUpdatePending } = useUpdateAttributes({
    onSuccess(data) {
      setAttributeList(data?.data);
    ...
    onError(error: axiosError) {
      const errorMessage =
        error?.response?.data?.errors?.message || error?.response?.data?.message || "update attributes failed";
      toast.error(errorMessage);
    },
  });
  const handleSave = () => {
  ...
  const { mutate: onDeleteAttribute, isPending: isPendingDelete } = useDeleteAttribute({
    onSuccess(data) {
      setTab(0);
      setAttributeList(data?.data);
      toast.success(data?.message);
    ...
    onError(error: axiosError) {
      const errorMessage =
        error?.response?.data?.errors?.message || error?.response?.data?.message || "delete attributes failed";
      toast.error(errorMessage);
    },
  });
  ...
  return (
    ...
    >
      {(loadFetchAttributes || isAddPending || isUpdatePending || refetchingAttributes || isPendingDelete) && (
      ...
      <div className="flex md:flex-row flex-col gap-4 md:gap-3 mt-0 md:mt-4 ">
          ...
          {tab === 0 ? (
            <div className="flex flex-col gap-3 md:gap-4">
              <div className="w-full">
                <label htmlFor="title" className="text-black font-normal text-lg">
                  Title
                </label>
                <Input
                  id="name"
              ...
              <div className="w-full">
                <label htmlFor="value" className="text-black font-normal text-lg">
                  Value
                </label>
                <Input
                  id="value"
            ...
            <div>
              <div className="flex items-center justify-between">
                ...
                <div className="flex gap-2 items-center">
                  ...
                  >
                    <BiEditAlt />
                    <span className="text-sm hidden md:block font-normal text-primary">Rename</span>
                  </div>
                  <div
                    className="bg-white p-2 rounded cursor-pointer flex items-center gap-1"
                  ...
                  >
                    ...
                    <span className="text-sm hidden md:block font-normal text-[#FF0000]">Delete</span>
                  </div>
                </div>
              </div>
        ...
        >
          ...
          <div className="flex items-center cursor-pointer gap-2 " onClick={() => setTab(0)}>
