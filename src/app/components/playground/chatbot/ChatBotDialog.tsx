...
import { IBotHistory, useGetChatbotResponse, useSaveContact } from "@/utils/chatbot-api";
...
const ChatBotDialog = ({
  ...
  const [closeChatPosition, setCloseChatPosition] = useState<"OFF" | "START" | "END">("OFF");
  ...
  const processNextMessage = async () => {
    ...
    await new Promise((resolve) => setTimeout(resolve, currentMessage.delay || 0));
  ...
  const { mutate: fetchBotResponse } = useGetChatbotResponse({
    ...
    onError(error: axiosError) {
      ...
      const errorMessage =
        error?.response?.data?.errors?.message || error?.response?.data?.message || "Failed to fetch bot response";
  ...
  useEffect(() => {
    if (!initialCallMade.current && chatbotId) {
      ...
      fetchBotResponse({
        ...
        type: "welcome-action",
        model: selectedModel,
        history: JSON.parse(localStorage.getItem(`${chatbotId}-history`) || "[]") as IBotHistory[],
      });
  ...
  const onTextSearch = () => {
    ...
    setPendingMessages((prev) => [...prev, { userInput: inputText, delay: 0, type: "user" }]);
    setApiLoading(true);
    fetchBotResponse({
      chatbotId: chatbotId as string,
  ...
  const onButtonSearch = (info: TypeButton) => {
    if (info.type === "url" && info.url) {
      ...
      setPendingMessages((prev) => [...prev, { userInput: info.title, delay: 0, type: "user" }]);
      setApiLoading(true);
      fetchBotResponse({
        chatbotId: chatbotId as string,
        type: "button-action",
  ...
  const { mutate: saveContact, isPending: isPendingToSave } = useSaveContact({
    ...
    onError(error: axiosError) {
      const errorMessage =
        error?.response?.data?.errors?.message || error?.response?.data?.message || "Failed to save contact";
      toast.error(errorMessage);
      setShowCloseDialog(false);
    },
  });
  ...
  console.log("chatbotName", chatbotName, botIcon, showCloseDialog, closeChatPosition);
  return (
    <div
    ...
    >
      ...
      <div className="flex flex-col h-full relative">
            ...
            <Select value={selectedModel} onValueChange={setSelectedModel}>
              ...
              <SelectContent>
                <SelectGroup>
                  ...
                  <SelectItem value="deepseek-v2:16b">deepseek-v2:16b</SelectItem>
            ...
            <TooltipProvider>
              <Tooltip>
                <TooltipTrigger asChild>
                  <Button
                    type="button"
                    className="!bg-transparent !rounded-lg !py-2 !px-4  transition-colors duration-300"
                    onClick={() => {
                ...
                <TooltipContent className="bg-[#1B1B20]" side="right" align="center" sideOffset={14}>
                  <p>Delete Chat history</p>
                </TooltipContent>
              </Tooltip>
            </TooltipProvider>
          </div>
          <Cross2Icon className="h-4 w-4 cursor-pointer" onClick={closeChatBotHandler} />
        </div>
        <div className="flex-1 bg-[#F1F1F1] p-4 overflow-y-auto show-scrollbar flex flex-col gap-3" ref={scrollRef}>
          {visibleMessages.map((item, index) => (
            <div key={index}>
              {item.type === "user" ? (
                ...
                <GalleryResponse info={item.info} onButtonSearch={onButtonSearch} />
              ) : item.type === "quick" && item.info ? (
                <QuickResponse info={item.info} onButtonSearch={onButtonSearch} />
              ) : item.type === "button" && item.info ? (
                <ButtonResponse info={item.info} onButtonSearch={onButtonSearch} />
        ...
        <div className="p-4 bg-white flex gap-4 items-center">
          ...
          />
          <IoSend className="text-[#7A7A7A] text-xl cursor-pointer" onClick={onTextSearch} />
        </div>
        {showCloseDialog && (
          <div className="absolute inset-0 bg-[#a6dae41a] flex items-center justify-center backdrop-blur-[1.3px]">
            <div className="bg-white rounded-xl p-4 w-[90%] max-w-[340px] mx-auto border border-[#53A7DD]/20 shadow-[0_8px_30px_rgb(0,0,0,0.12)] hover:shadow-[0_8px_30px_rgb(83,167,221,0.12)] transition-shadow duration-300">
              <div className="flex justify-between items-center mb-4">
                <h3 className="text-lg font-semibold text-[#1E255E]">Please Enter Details</h3>
                <IoClose className="text-[#1E255E] text-xl cursor-pointer" onClick={closeChatHandler} />
              </div>
