...
export const ImageResponse = ({ info }: { info: ResponseInfo }) => {
  return (
    <div className="w-9/12 h-64">
      <Image src={info.file || ""} alt="Selected" layout="fill" objectFit="cover" priority className="rounded" />
    </div>
  );
};
...
export const UserInput = ({ data }: { data: TypeBotResponse }) => {
  return (
    <div className="flex flex-col gap-[5px] w-fit ms-auto">
      ...
      <div className="bg-[#57C0DD] p-3 rounded-lg text-white font-light  text-sm w-fit">{data?.userInput ?? ""}</div>
    </div>
  );
};
