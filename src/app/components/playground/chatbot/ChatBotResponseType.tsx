...
export const LlmResponse = ({ info }: { info: ResponseInfo }) => {
  const preservePhoneNumbers = (text: string) => {
    // Fix existing markdown tel links by removing spaces/hyphens
    const cleanedMarkdownLinks = text.replace(/\[([^\]]*?)\]\(tel:([^)]+?)\)/g, (_, label, tel) => {
      const cleanedTel = tel.replace(/\s|-/g, "");
      return `[${label}](tel:${cleanedTel})`;
    });

    // Wrap raw phone numbers that aren't already linked in backticks
    const finalWithBackticks = cleanedMarkdownLinks.replace(/(?<!\[)(\+?\d[\d\s-]{7,})(?!\]\(tel:.*?\))/g, (match) => {
      return `\`${match.trim()}\``;
    });

    return finalWithBackticks;
  };
    ...
    <div className="w-full">
      <div className="w-10/12 ms-auto">
        <div className="flex flex-col gap-[5px] w-fit  ms-auto ">
          ...
          <div className="bg-[#57C0DD] p-3 rounded-md text-white font-light  text-sm">{data?.userInput ?? ""}</div>
        </div>
      </div>
    </div>
  );
