...
const DashboardLayout = ({ children }: { children: React.ReactNode }) => {
  ...
  const showUser =
    role === 'admin' ||
    (role !== 'user' &&
      !(role === 'subadmin' && !permissions.includes('MANAGE_USERS')));
  const showBda =
    role === 'admin' ||
    (role !== 'user' &&
      !(
        role === 'subadmin' && !permissions.includes('BDA_QUESTION_MANAGEMENT')
      ));
  const showStatistics =
    role === 'admin' ||
    (role !== 'user' &&
      !(role === 'subadmin' && !permissions.includes('ADMIN_DASHBOARD')));
  const showLlm = role === 'admin';
  const showChatbotLisAndStatistics =
    role === 'admin' ||
    role === 'user' ||
    !(role === 'subadmin' && !permissions.includes('CREATE_CHATAGENTS'));
  return (
    <UserRoleProvider>
