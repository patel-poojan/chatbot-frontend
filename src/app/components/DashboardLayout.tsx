...
import { MdKeyboardArrowRight, MdLockReset, MdOutlineLogout, MdOutlinePeopleAlt, MdShowChart } from "react-icons/md";
...
const DashboardLayout = ({ children }: { children: React.ReactNode }) => {
  ...
  const showUser =
    role === "admin" || (role !== "user" && !(role === "subadmin" && !permissions.includes("MANAGE_USERS")));
  const showBda =
      ...
      <div className="min-[500px]:bg-[#1B1B20] h-dvh  flex  p-0 min-[500px]:p-3">
              ...
              <Tooltip>
                <TooltipTrigger asChild>
                  <Link href={"/statistics-dashboard-user"}>
                    <div
                      className={`${
                        pathName === "/statistics-dashboard-user" ? "bg-[#3D3D4A]" : "bg-transparent"
                ...
                <Tooltip>
                  <TooltipTrigger asChild>
                    <Link href={"/statistics-dashboard"}>
                      <div
                        className={`${
                          pathName === "/statistics-dashboard" ? "bg-[#3D3D4A]" : "bg-transparent"
                        } hover:bg-[#3D3D4A] h-11 w-11 flex items-center justify-center rounded-md cursor-pointer`}
                      >
              ...
              <Tooltip>
                <TooltipTrigger asChild>
                  <Link href={"/chatbotlist"}>
                    <div
                      className={`${
                        pathName === "/chatbotlist" || pathName === "/create" || pathName.includes("/dashboard")
                          ? "bg-[#3D3D4A]"
                          : "bg-transparent"
                      } hover:bg-[#3D3D4A] h-11 w-11 flex items-center justify-center rounded-md cursor-pointer`}
                    >
                ...
                <TooltipContent side="right" align="center" className="bg-[#1B1B20]" sideOffset={14}>
                  <p>ChatAgents</p>
                </TooltipContent>
              </Tooltip>
            </TooltipProvider>
        ...
        <div
          className={`flex-1 w-full flex flex-col bg-white ${
            pathName.startsWith("/dashboard") ? "p-0" : "max-[500px]:p-0 p-6 lg:p-8 xl:px-16 xl:py-12"
                  ...
                  <div className="w-full  flex-1">
                    <Link href={"/statistics-dashboard-user"}>
                      <div
                        className={`flex gap-3 items-center rounded-lg py-3 ${
                          pathName === "/statistics-dashboard-user" ? "px-3 blue-gradient" : ""
                        }`}
                      >
                        <div
                          className={pathName === "/statistics-dashboard-user" ? "p-2 rounded-md bg-[#3D3D4A33] " : ""}
                        >
                          <MdShowChart
                            className={
                        ...
                        <p
                          className={`text-base ${
                            pathName === "/statistics-dashboard-user" ? "text-white" : "text-[#1e255eb2]"
                          } font-medium`}
                        >
                          User Statistics
                        </p>
                    ...
                    {showStatistics && (
                      <Link href={"/statistics-dashboard"}>
                        <div
                          className={`flex gap-3 items-center rounded-lg py-3 ${
                            pathName === "/statistics-dashboard" ? "px-3 blue-gradient" : ""
                          }`}
                        >
                          <div className={pathName === "/statistics-dashboard" ? "p-2 rounded-md bg-[#3D3D4A33] " : ""}>
                            <IoStatsChart
                              className={
                          ...
                          <p
                            className={`text-base ${
                              pathName === "/statistics-dashboard" ? "text-white" : "text-[#1e255eb2]"
                            } font-medium`}
                          >
                            Statistics
                          </p>
                    ...
                    {showUser && (
                      <Link href={"/users"}>
                        ...
                        >
                          <div className={pathName === "/users" ? "p-2 rounded-md bg-[#3D3D4A33] " : ""}>
                            <MdOutlinePeopleAlt
                              className={pathName === "/users" ? "text-white text-xl" : "text-2xl text-[#1e255eb2]"}
                            />
                          </div>
                          <p
                            className={`text-base  ${
                              pathName === "/users" ? "text-white" : "text-[#1e255eb2]"
                            }  font-medium`}
                          >
                            User
                          </p>
                    ...
                    {showBda && (
                      <Link href={"/bda"}>
                        ...
                        >
                          <div className={pathName === "/bda" ? "p-2 rounded-md bg-[#3D3D4A33] " : ""}>
                            <PiNotepadBold
                              className={pathName === "/bda" ? "text-white text-xl" : "text-2xl text-[#1e255eb2]"}
                            />
                          </div>
                          <p
                            className={`text-base  ${
                              pathName === "/bda" ? "text-white" : "text-[#1e255eb2]"
                            }  font-medium`}
                          >
                            BDA
                          </p>
                      ...
                      <div className="flex justify-between items-center border-2 bg-[#e9e9e933] rounded-xl border-[#F3F3F3] p-2">
                        <div className="flex items-center gap-3">
                          ...
                          <div className="text-start">
                            <p className="text-[#1e255eb2] font-medium text-base capitalize">{userName ?? "User"}</p>
                            <p className="text-[#1e255eb2] font-light text-base">{email ?? "Email"}</p>
                          </div>
                        </div>
