...
const BDAQuestion = ({
  ...
  setQuestionAnswer: React.Dispatch<React.SetStateAction<{ question: string; answer: string }[]>>;
  errorInBDAQuestionList: boolean;
}) => {
  const { mutate: onSave, isPending: savePending } = useSaveBDAQuestion({
    ...
    onError(error: axiosError) {
      const errorMessage = error?.response?.data?.errors?.message || error?.response?.data?.message || "failed to save";
  ...
  return (
    ...
    >
      <div className="w-full max-w-7xl flex-1 mx-auto h-auto flex flex-col overflow-hidden">
        ...
        <div className="text-black text-lg md:text-xl font-normal my-4">Answer the following questions</div>
        <div className="flex flex-col gap-4 md:gap-6 flex-1 overflow-scroll p-[2px]">
          {!errorInBDAQuestionList ? (
            questionAnswer.length > 0 ? (
              ...
              >
                ...
                <p className="text-sm text-gray-500 text-center">
                  There are currently no questions available for this industry and sub-industry combination.
                </p>
              </div>
            )
          ) : (
