...
const Page = () => {
  ...
  const { mutate: onLogin, isPending } = useLogin({
    onSuccess(data) {
      ...
      if (token) {
        ...
        console.log("data.data.user.permissions", data.data.user.permissions);
        if (data.data.user.permissions) {
          Cookies.set("permissions", JSON.stringify(data.data.user.permissions), {
            path: "/",
            sameSite: "Lax",
            secure: true,
          });
        ...
        if (userRole === "user") {
          ...
          if (data.data.user.permissions && data.data.user.permissions.includes("ADMIN_DASHBOARD")) {
            router.push("/statistics-dashboard");
          } else {
            router.push("/statistics-dashboard-user");
          }
    ...
    onError(error: axiosError) {
      const errorMessage = error?.response?.data?.errors?.message || error?.response?.data?.message || "Login failed";
  ...
  return (
    <div className="h-dvh w-dvw flex justify-center items-center">
      {isPending && <Loader />}
      <form onSubmit={handleSubmit} className="flex flex-col gap-4 md:gap-6 w-full max-w-lg px-4">
        <div className="flex flex-col gap-1 md:gap-2 justify-center items-center">
          ...
          <p className="text-black font-medium text-2xl md:text-[32px]">Welcome back</p>
        </div>
        ...
        <div className="text-center text-[#1E255EB2] font-normal text-sm sm:text-lg">
          ...
          <a href="signup" className="text-[#57C0DD] ms-1 hover:text-[#45A9B8] underline-offset-2 hover:underline">
            Sign up
          </a>
        </div>
      </form>
