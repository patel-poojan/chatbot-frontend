...
const Page = () => {
  ...
  const fetchChatbotList = async () => {
    const response: FetchChatbotListResponse = await axiosInstance.get(`/chatbot`);
    if (response.success) {
  ...
  const { mutate: onDelete, isPending: deletePending } = useDeleteBot({
    ...
    onError(error: axiosError) {
      const errorMessage =
        error?.response?.data?.errors?.message || error?.response?.data?.message || "Delete bot failed";
  ...
  const { mutate: onUpdateBot, isPending: updatePending } = useUpdateChatbot({
    ...
    onError(error: axiosError) {
      const errorMessage = error?.response?.data?.errors?.message || error?.response?.data?.message || "Rename failed";
      toast.error(errorMessage);
    },
  });
    ...
    <div className="flex-1 flex flex-col max-[500px]:p-4 overflow-auto">
      ...
      <div className="mb-8">
        <h1 className="text-2xl font-medium text-gray-700">
          {greeting},<span className="capitalize">{userName ? ` ${userName}! 👋` : ""}</span>
        </h1>
        <p className="text-gray-500 mt-1">Here are your ChatAgents</p>
      </div>
      <div className="flex items-center justify-between">
        <div className="max-[500px]:text-xl text-2xl font-semibold text-black mb-6 max-[500px]:mb-4">ChatAgents</div>
      ...
      <div className="flex-1 overflow-y-auto">
        <div className="grid gap-4 sm:gap-6 grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 2xl:grid-cols-5">
          ...
          >
            ...
            <div className="text-white mt-1 font-medium text-sm sm:text-base">Add ChatAgent</div>
          </div>
          {!errorInChatbotList &&
            filteredChatbotList &&
                ...
                <div className="flex justify-between gap-5 items-center">
                  {editingId === data._id ? (
                    <Input
                      ...
                      onKeyDown={(e) => e.key === "Enter" && handleRenameSubmit(data._id)}
                    ...
                    <span className="text-lg truncate font-medium text-black">{data.name}</span>
                  )}
                  <Popover>
