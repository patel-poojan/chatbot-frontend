...
const ForgetPassword = () => {
  ...
  const { mutate: onForgetPassword, isPending: isForgetPasswordPending } = useForgetPassword({
    onSuccess(data) {
      toast.success(data?.message);
    },
    onError(error: axiosError) {
      const errorMessage =
        error?.response?.data?.errors?.message || error?.response?.data?.message || "forget password failed";
      toast.error(errorMessage);
    },
  });
  const { mutate: onForgetPasswordReset, isPending: isForgetPasswordResetPending } = useForgetPasswordReset({
    onSuccess(data) {
      router.push("/login");
      toast.success(data?.message);
    },
    onError(error: axiosError) {
      const errorMessage =
        error?.response?.data?.errors?.message || error?.response?.data?.message || "Reset password failed";
      toast.error(errorMessage);
    },
  });
  ...
  return (
    <div className="min-h-dvh flex items-center justify-center p-4 ">
      ...
      {token ? (
        ...
        >
          <h1 className="text-black font-medium text-2xl md:text-[32px]  text-center">Forgot Your Password</h1>
          <div className="w-full">
            <label htmlFor="newPassword" className="text-black font-normal text-lg">
              New Password
            </label>
            <div className="flex items-center pe-4 border w-full rounded mt-1">
              ...
              <div className="cursor-pointer" onClick={toggleNewPassword}>
                {newPasswordType === "password" ? <IoEyeOffOutline /> : <IoEyeOutline />}
              </div>
            </div>
          </div>
          <div className="w-full">
            <label htmlFor="confirmPassword" className="text-black font-normal text-lg">
            ...
            <div className="flex items-center pe-4 border w-full rounded mt-1">
              ...
              <div className="cursor-pointer" onClick={toggleConfirmPassword}>
                {confirmPasswordType === "password" ? <IoEyeOffOutline /> : <IoEyeOutline />}
              </div>
            </div>
          </div>
          <Button
        ...
        <form
          ...
          style={{ boxShadow: "0px 0px 12px 4px #00000014" }}
        >
          <h1 className="text-black font-medium text-2xl md:text-[32px]">Forgot Your Password ?</h1>
